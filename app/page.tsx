    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        ...
        <div className="max-w-7xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="#campaigns">
                <button className="btn-primary text-lg px-8 py-3">Explore Campaigns</button>
              </a>
              <a href="/create">
                <button className="btn-secondary text-lg px-8 py-3">Start a Campaign</button>
              </a>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          {[
            { icon: Heart, color: 'primary', label: 'Total Campaigns', value: stats.totalCampaigns },
            { icon: Coins, color: 'green', label: 'Total Raised (ALGO)', value: microToAlgo(stats.totalRaised) },
            { icon: TrendingUp, color: 'orange', label: 'Active Campaigns', value: stats.activeCampaigns },
            { icon: Users, color: 'purple', label: 'Donors', value: '1000+' },
      ...
      <section id="campaigns" className="py-16 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          {appId === 0 ? (
            <div className="text-center py-20 bg-yellow-50 rounded-xl border border-yellow-200">
              <p className="text-yellow-800 text-lg font-medium">App not deployed yet.</p>
              <p className="text-yellow-600 mt-2">Set <code className="bg-yellow-100 px-1 rounded">NEXT_PUBLIC_ALGORAND_APP_ID</code> in your <code className="bg-yellow-100 px-1 rounded">.env.local</code> after deploying the contract.</p>
            </div>
          ) : loading ? (
            ...
            <div className="text-center py-20">
              <p className="text-gray-600 text-lg">No active campaigns yet. Be the first to create one!</p>
            </div>
          ) : (
